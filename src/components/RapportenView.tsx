...
import {
  ...
  genereerFacturenRapport,
  type VerbruikRapportRegel,
  type LedgerRapportRegel,
  type FactuurRapportRegel,
} from "@/app/(app)/rapporten/actions";

type RapportType = "verbruik" | "ledger" | "facturen";
...
export default function RapportenView({ buildings }: { buildings: Building[] }) {
  ...
  const [facturenRows, setFacturenRows] = useState<FactuurRapportRegel[] | null>(null);
  ...
  function genereer() {
    startTransition(async () => {
      ...
      setLedgerRows(null);
      setFacturenRows(null);
      if (rapportType === "verbruik") {
        setVerbruikRows(await genereerVerbruikRapport(buildingId, vanDatum, totDatum));
      } else if (rapportType === "ledger") {
        setLedgerRows(await genereerLedgerRapport(buildingId, vanDatum, totDatum));
      } else {
        setFacturenRows(await genereerFacturenRapport(buildingId, vanDatum, totDatum));
      }
    });
  }
  ...
  function csv() {
    if (rapportType === "verbruik" && verbruikRows) {
      exportCsv(
        ...
        verbruikRows.map((r) => [formatDate(r.datum), r.gebouw, r.huurder, r.activiteit, r.drankTotaal.toFixed(2), r.boetesTotaal.toFixed(2), r.totaal.toFixed(2)])
      );
    } else if (rapportType === "ledger" && ledgerRows) {
      exportCsv(
    ...
    } else if (facturenRows) {
      exportCsv(
        `facturen_${vanDatum}_${totDatum}`,
        ["Datum", "Gebouw", "Naam", "Type", "Bedrag", "Wie"],
        facturenRows.map((r) => [formatDate(r.datum), r.gebouw, r.naam, r.type, r.bedrag.toFixed(2), r.wie])
      );
    }
  }
  ...
  function excel() {
    if (rapportType === "verbruik" && verbruikRows) {
      exportExcel(
        `verbruiksrapport_${vanDatum}_${totDatum}`,
        ["Datum", "Gebouw", "Huurder", "Activiteit", "Drank", "Boetes", "Totaal"],
        verbruikRows.map((r) => [formatDate(r.datum), r.gebouw, r.huurder, r.activiteit, r.drankTotaal, r.boetesTotaal, r.totaal])
      );
    } else if (rapportType === "ledger" && ledgerRows) {
      exportExcel(
        `voorraadbewegingen_${vanDatum}_${totDatum}`,
      ...
      exportExcel(
        `facturen_${vanDatum}_${totDatum}`,
        ["Datum", "Gebouw", "Naam", "Type", "Bedrag", "Wie"],
        facturenRows.map((r) => [formatDate(r.datum), r.gebouw, r.naam, r.type, r.bedrag, r.wie])
      );
    }
  }

  const heeftResultaat =
    (verbruikRows && verbruikRows.length > 0) || (ledgerRows && ledgerRows.length > 0) || (facturenRows && facturenRows.length > 0);
  const verbruikTotaal = verbruikRows?.reduce((s, r) => s + r.totaal, 0) ?? 0;
  const facturenTotaal = facturenRows?.reduce((s, r) => s + (r.type === "Factuur" ? r.bedrag : -r.bedrag), 0) ?? 0;

  return (
    <div>
      <div className="print:mb-4">
        <h1 className="text-2xl font-bold text-[#171A2B] mb-1">Rapportagecentrum</h1>
        <p className="text-[#8A8FA8] text-sm mb-6 print:hidden">Stel een periode en gebouw samen en exporteer als CSV, Excel of PDF.</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#ECECF3] p-5 mb-6 print:hidden">
        <div className="grid sm:grid-cols-3 gap-2 mb-4">
          <button
            ...
            className={`py-2.5 rounded-lg text-sm font-semibold border ${rapportType === "verbruik" ? "bg-[#6D5AE6] text-white border-[#6D5AE6]" : "bg-white text-[#171A2B] border-[#ECECF3]"}`}
          >
            Verbruik per reservatie
          </button>
          <button
            onClick={() => setRapportType("ledger")}
            className={`py-2.5 rounded-lg text-sm font-semibold border ${rapportType === "ledger" ? "bg-[#6D5AE6] text-white border-[#6D5AE6]" : "bg-white text-[#171A2B] border-[#ECECF3]"}`}
          >
            Leveringen, verbruik & verplaatsingen
          </button>
          <button
            onClick={() => setRapportType("facturen")}
            className={`py-2.5 rounded-lg text-sm font-semibold border ${rapportType === "facturen" ? "bg-[#6D5AE6] text-white border-[#6D5AE6]" : "bg-white text-[#171A2B] border-[#ECECF3]"}`}
          >
        ...
        <div className="grid sm:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <input type="date" value={vanDatum} onChange={(e) => setVanDatum(e.target.value)} className="w-full mt-1.5 rounded-lg border border-[#ECECF3] px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="text-xs font-semibold text-[#8A8FA8] uppercase tracking-wide">Tot en met</label>
            <input type="date" value={totDatum} onChange={(e) => setTotDatum(e.target.value)} className="w-full mt-1.5 rounded-lg border border-[#ECECF3] px-3 py-2 text-sm" />
          </div>
          <div className="flex items-end">
            <button onClick={genereer} disabled={pending} className="w-full py-2 rounded-lg bg-[#6D5AE6] text-white text-sm font-semibold disabled:opacity-50">
      ...
      {(verbruikRows || ledgerRows || facturenRows) && (
        <div className="bg-white rounded-2xl border border-[#ECECF3] overflow-hidden">
          <div className="px-5 py-4 border-b border-[#ECECF3] flex items-center justify-between print:border-none">
            <div>
              <div className="font-bold text-[#171A2B]">
                {rapportType === "verbruik" ? "Verbruik per reservatie" : rapportType === "ledger" ? "Leveringen, verbruik & verplaatsingen" : "Facturen & creditnota's"}
              </div>
          ...
          {!heeftResultaat && <div className="px-5 py-8 text-sm text-[#B0B4CC] text-center">Geen resultaten voor deze selectie.</div>}
          ...
          {rapportType === "facturen" && facturenRows && facturenRows.length > 0 && (
            <table className="w-full text-sm">
              <thead className="bg-[#F7F7FB] text-[#8A8FA8] text-xs uppercase print:bg-transparent">
                <tr>
                  <th className="text-left px-5 py-2.5 font-semibold">Datum</th>
                  <th className="text-left px-3 py-2.5 font-semibold">Gebouw</th>
                  <th className="text-left px-3 py-2.5 font-semibold">Naam</th>
                  <th className="text-left px-3 py-2.5 font-semibold">Type</th>
                  <th className="text-right px-3 py-2.5 font-semibold">Bedrag</th>
                  <th className="text-left px-5 py-2.5 font-semibold">Wie</th>
                </tr>
              </thead>
              <tbody>
                {facturenRows.map((r, i) => (
                  <tr key={i} className="border-t border-[#ECECF3]">
                    <td className="px-5 py-2.5 text-[#5B5F82]">{formatDate(r.datum)}</td>
                    <td className="px-3 py-2.5 text-[#5B5F82]">{r.gebouw}</td>
                    <td className="px-3 py-2.5 font-medium text-[#171A2B]">{r.naam}</td>
                    <td className="px-3 py-2.5 text-[#171A2B]">{r.type}</td>
                    <td className={`px-3 py-2.5 text-right font-semibold ${r.type === "Creditnota" ? "text-[#D6493C]" : "text-[#171A2B]"}`}>
                      {r.type === "Creditnota" ? "\u2212" : ""}&euro;{r.bedrag.toFixed(2)}
                    </td>
                    <td className="px-5 py-2.5 text-[#5B5F82]">{r.wie || "—"}</td>
                  </tr>
                ))}
              ...
              <tfoot>
                <tr className="border-t-2 border-[#ECECF3]">
                  <td colSpan={4} className="px-5 py-3 text-right font-semibold text-[#171A2B]">
                    Totaal (factuur &minus; creditnota)
                  </td>
                  <td colSpan={2} className="px-5 py-3 text-right font-bold text-[#171A2B]">&euro;{facturenTotaal.toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>
          )}
