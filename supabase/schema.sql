...
-- Nieuwe auth-gebruiker krijgt automatisch een profiel. Staat er al een vooraf
-- klaargezette rol klaar voor dit e-mailadres (pending_roles), dan wordt die
-- overgenomen (inclusief toegewezen gebouwen); anders krijgt de gebruiker de
-- standaardrol "administratie", aan te passen via "Beheer & rollen".
...
as $$
declare
  v_pending pending_roles%rowtype;
begin
  select * into v_pending from pending_roles where lower(email) = lower(new.email) limit 1;

  if found then
    insert into public.profiles (id, email, full_name, role)
    values (new.id, new.email, coalesce(v_pending.full_name, new.raw_user_meta_data ->> 'full_name', new.email), v_pending.role);

    insert into public.profile_buildings (profile_id, building_id)
    select new.id, building_id from pending_role_buildings where pending_id = v_pending.id;

    delete from pending_roles where id = v_pending.id;
  else
    insert into public.profiles (id, email, full_name, role)
    values (new.id, new.email, coalesce(new.raw_user_meta_data ->> 'full_name', new.email), 'administratie');
  end if;
